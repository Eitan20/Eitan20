...
import { checkValidQueryAndStyle, parseHeaderStr, parseKeywordMultiStr } from './input-validation.js'
...
/* 
Select a subset of a user's additional keywords 
in the event that including all of them
(a) takes too much tokens / space in the input query
(b) or for variation, so it doesn't repeat too often

Select based off the frequency of the keyword phrases in article generated so far

keywordsAdditionalCounts: 
	map phrase to occurrence count in the article for all previously generated sections
*/
function selectKeywordsAdditionalSubset(sectionIdx, paragraphsPerSection, keywordsAdditionalCounts, debug) {
	
	let paragraphsPerSectionMax = 5

	if (paragraphsPerSection < 1 || paragraphsPerSection > paragraphsPerSectionMax) {
		console.warn(`Expected paragraphsPerSection 1 <= p <= ${paragraphsPerSectionMax}`)
	}

	// Scale the max frequency count with the article length param
	// and potentially the total number of keywords
	// but also the number of sections is variable now, hmmm

	//let minOccurrence = 1;
	let maxOccurrence = 2;
	let boostForLength = 5
	let p = Math.max(1, Math.min(paragraphsPerSection, paragraphsPerSectionMax))
	let boost = parseInt((p / paragraphsPerSectionMax) * boostForLength)
	maxOccurrence += boost

	// Limit total number of phrases returned per section
	let maxNumPhrasesPerSection = 20

	// Limit total number of tokens used by the keyword phrases because they take space in the prompt
	let maxNumTokensPerSection = 500

	if (debug) console.log(`Section ${sectionIdx}, additional kw maxOccurrence: ${maxOccurrence}`)
	...
	let tokenCount = 0
	let subset = []
	let kwsShuffled = shuffle(Object.keys(keywordsAdditionalCounts))

	// Start with random subset of keywords with 0 occurrences
	kwsShuffled.forEach((keywordPhrase) => {
		
		let count = keywordsAdditionalCounts[keywordPhrase]
		
		if (count === 0 && 
			subset.length < maxNumPhrasesPerSection && 
			tokenCount + numTokens(keywordPhrase) < maxNumTokensPerSection
		) {
			tokenCount += numTokens(keywordPhrase)
			subset.push(keywordPhrase)
		}
	})

	// Add remaining keywords if possible
	kwsShuffled.forEach((keywordPhrase) => {
	...
	if (debug) console.log(`Section ${sectionIdx}, toks used: ${tokenCount}, kw phrases: `, subset, keywordsAdditionalCounts)

	return {
		keywords: subset,
		estimatedTokensUsed: tokenCount
	}
}

/*
If using keywords feature, update the occurrence counts
Even for words or phrases that occur naturally, unprompted
*/
function updateKeywordPhraseCounts(keywordsAdditionalCounts, textResponse, debug) {
	
	let textLower = textResponse.toLowerCase()

	Object.keys(updateKeywordPhraseCounts).forEach((phrase) => {

		let count = keywordsAdditionalCounts[phrase]
		let phraseLower = phrase.toLowerCase()
		let additions = textLower.match(new RegExp(phraseLower, "g")).length // TODO: Case insensitive???
		if (additions > 0) keywordsAdditionalCounts[phrase] = count + additions
	})

	if (debug) console.log("kw counts after update: ", keywordsAdditionalCounts)
}

/*
Returns a prompt string to append to the main prompt for the section

keywordSubset: list of keyword phrases to increase the occurrence / frequency
*/
function getPromptAdditionalKeywords(keywordSubset) {
	
	let n = keywordSubset.length

	if (n === 0) return ""

	let base;

	// TODO: This also has to account for short sections of 1 paragraph, up to multiple subsections in a section with 5 paragraphs each
	// Might want to limit the overall list

	base += " If thematically appropriate and natural sounding, "

	if (n === 1) base += "include the phrase: "
	else if (n < 5) base += "include these phrases where possible: "
	else {
		base += "include several phrases from the following: " // \n
	}

	let prompt = `${base} ${keywordSubset.join(", ")}.`
...
*/
function getPromptMainOutline(sectionIdx, nSections, queryMain, sectionHeader, 
	idStyle, paragraphsPerSection, useMarkdown, outlineIsUserInput) {
	...
	let base = `The outline references sections of a ${idToArticleStyle[idStyle].toLowerCase()} article about ${queryMain}. ` + 
		`Write section ${sectionIdx+1} / ${nSections} of the article that covers these subtopics:\n` + 
		`${sectionHeader}\n\n`
	...
	// Do not generate titles or try to add html if user supplied headers
	if (useMarkdown && !outlineIsUserInput) {
	...
	if (!outlineIsUserInput) {
		...
		contentDescription += " Do not write any section titles or subtitles, and do not number the sections in any way."
	}
...
export class GPTWrapper {
	...
	headerStr: string, user can provide optional individual section titles / topics, newline delimited or comma delimited
	keywordMultiInput: user can provide optional list of keyword phrases to increase the presence and frequency of
	*/
	async WriteArticle_(keywordPhrase, idStyle, paragraphsPerSection, headerStr, keywordMultiInput, updateProgress) {
		...
		return await this.WriteArticleUsingOutline_(keywordPhrase, idStyle, paragraphsPerSection, headerStr, keywordMultiInput, updateProgress)
	}
	...
	*/
	async WriteArticleUsingOutline_(keywordPhrase, idStyle, paragraphsPerSection, headerStr, keywordMultiInput, updateProgress) {
		...
		const headers = parseHeaderStr(headerStr)
		const keywordsAdditional = parseKeywordMultiStr(keywordMultiInput)

		// 10 is just an estimated number of sections returned from the outline
		let progressMax = headers.length > 0 ? headers.length : 10 
		...
		let outline;
		let outlineIsUserInput = false;

		if (headers.length === 0) {
			outline = await this.GetOutline_(keywordPhrase.trim(), idStyle, paragraphsPerSection, useMarkdown, keywordsAdditional)
		}
		else {
			outline = await this.GetOutlineFromUserInput_(keywordPhrase.trim(), idStyle, paragraphsPerSection, useMarkdown, headers, keywordsAdditional)
			outlineIsUserInput = true;
		}
		...
		return await this.SendOutline_(updateProgress, includeMetaDescription, includeTitle, includeMetaTitle, outlineIsUserInput)
	}
	...
	async GetOutline_(q, idStyle, paragraphsPerSection, useMarkdown, keywordsAdditional) { 
		...
		if (!resultObj) {
			...
			if (!resultLower.includes("conclusion") && !resultLower.includes("summary") && !resultLower.includes("recap")) { // "closing"
				resultObj.result = `${resultObj.result.trimEnd()}\nConclusion\n`
			}

			// Save original input params used later
			resultObj.queryOriginal = q;
			resultObj.idStyle = idStyle
			resultObj.paragraphsPerSection = paragraphsPerSection
			resultObj.keywordsAdditional = keywordsAdditional
			this.results_.push(resultObj)
			this.TokenizeOutline_(resultObj)
			this.queryActive_ = false;
			let outlineIsUserInput = false
			return this.GetPromptsForSections(resultObj, idStyle, paragraphsPerSection, useMarkdown, outlineIsUserInput)
		}
	}

	/* Use user input for outlined sections instead of generating */
	async GetOutlineFromUserInput_(q, idStyle, paragraphsPerSection, useMarkdown, headers, keywordsAdditional) { 
			
		let resultObj = {
			queryOriginal: q,
			idStyle: idStyle,
			paragraphsPerSection: paragraphsPerSection,
			keywordsAdditional: keywordsAdditional,
			result: headers.join("\n"),
			answers: []
		}
		...
		// Each outline subsection will be 1 header
		resultObj.sections = resultObj.result.split(/[\n]+/).filter((s) => s.trim() !== "").map((text) => (
			{ text: text }
		));

		this.queryActive_ = false;
		const outlineIsUserInput = true
		return this.GetPromptsForSections(resultObj, idStyle, paragraphsPerSection, useMarkdown, outlineIsUserInput)
	}

	GetPromptsForSections(resultObj, idStyle, paragraphsPerSection, useMarkdown, outlineIsUserInput) {
		// Update prompts section
		let i = 0
		let nSections = resultObj.sections.length
		resultObj.sections.forEach((section) => {
			if (this.debug_) console.log("Getting prompt for section: ", section)
			// If outlineIsUserInput .original will be the header created by the user for the section
			section.original = section.text;
			section.text = getPromptMainOutline(i, nSections, resultObj.queryOriginal, 
				section.text, idStyle, paragraphsPerSection, useMarkdown, outlineIsUserInput)
			i += 1
		})
	...
	*/
	...
	PostProcess_(resultObj, outlineIsUserInput) {
		
		if (this.debug_) console.log("Before postproc: ", resultObj.answers);

		let sections = []
		let sectionsTitlesFromUser = []
		let headersFromUser = resultObj.sections.map((s) => s.original)

		if (outlineIsUserInput && headersFromUser.length > 0 && headersFromUser.length !== resultObj.answers.length) {
			console.error("Headers from user len !== sections len, maybe one dropped", resultObj.answers.length, headersFromUser.length, headersFromUser)
		}
		...
		resultObj.answers.forEach((a) => {
			...
			sents.forEach((sent) => {
				...
				// Strip any repetition of the title
				// When user provides titles, model may start paragraph by repeating the topic / title
				// Now including in the prompt to use sentences only, never write titles for this parameter.
				if (outlineIsUserInput) {
					let titleForSectionLower = headersFromUser[idx].toLowerCase()

					if (sentLower.startsWith(titleForSectionLower)) {
						
						sentCleaned =  sentCleaned.slice(titleForSectionLower.length)

						if (sentCleaned.startsWith(":") || sentCleaned.startsWith("-")) {
							// Drop any following spaces
							sentCleaned = sentCleaned.slice(1).trimStart()
						}
						else if (sentCleaned.startsWith(" : ") || sentCleaned.startsWith(" - ")) {
							sentCleaned = sentCleaned.slice(3)
						}
					}
				}
				...
				ts.forEach((s) => {
					if (sentLower.startsWith(s)) {
						...
						else sentCleaned = sentCleaned.slice(s.length).trimStart()
			...
			/*
			The section will continue titles and subtitles usually, but
			if the user inputs their own headers, then generally there will be no titles at this point in the text, 
			they get prepended after this loop
			*/
			// Test case, add to test later
			// let section = [
			...
			if (section.length > 0) {
				sections.push(section)
				if (outlineIsUserInput) sectionsTitlesFromUser.push(headersFromUser[idx])
			}
		...
		if (this.debug_) {
			if (outlineIsUserInput) console.log("Final sections: ", sections, sectionsTitlesFromUser)
			else console.log("Final sections: ", sections)
		}

		// Each section is a list of sents that also contains <p> open and close tags, 
		// but header tags are within the sentence text themselves right now (on quick article workflow only)
		let articleBody = ""
		idx = 0
		sections.forEach((s) => {
			...
			if (outlineIsUserInput) { 
				articleBody += `<h2>${sectionsTitlesFromUser[idx]}</h2>`
			}
		...
		*/
		...
		// Prepend the user's input keyword phrase and style
		metadata += "<p><strong>Prompt</strong></p><p>" + resultObj.queryOriginal.trim() + pCloseTag
		metadata += "<p><strong>Style</strong></p><p>" + idToArticleStyle[resultObj.idStyle] + pCloseTag
		...
		if (resultObj.hasOwnProperty('metaTitle')) {
			...
			metadata += "<p><strong>Meta Title</strong></p><p>" + metaTitle + pCloseTag
		}
		...
		if (resultObj.hasOwnProperty('metaDescription')) {
			...
			metadata += "<p><strong>Meta Description</strong></p><p>" + metaDescription + pCloseTag
		}
	...
	*/
	async SendOutline_(updateProgress, includeMetaDescription, includeTitle, includeMetaTitle, outlineIsUserInput) {
		...
		*/
		...
		// Clean prefixes and limit the length of the outline if not from the user
		if (!outlineIsUserInput) {
			outline = outline
				.split('\n')
		...
		// Track the counts of keyword phrases input by the user (and increase frequency)
		if (resultObj.keywordsAdditional.length > 0) {
			const counts = {}
			resultObj.keywordsAdditional.forEach((kw) => { counts[kw] = 0 })
			resultObj.keywordsAdditionalCounts = counts
		}
		...
		for (let i=0; i<qs.length; i++) {
			...
			// Modify the prompt to include additional keywords
			if (resultObj.keywordsAdditional.length > 0) {
				let kwSubset = selectKeywordsAdditionalSubset(
					i, resultObj.paragraphsPerSection, resultObj.keywordsAdditionalCounts, this.debug_)

				if (kwSubset.keywords.length > 0) {
					q += getPromptAdditionalKeywords(kwSubset.keywords)
				}
			}
			...
			try {
				...
				*/
				...
				// If using keywords feature, update the occurrence counts
				if (resultObj.keywordsAdditional.length > 0) updateKeywordPhraseCounts(resultObj.keywordsAdditionalCounts, answer.result, this.debug_)
			}
			catch (error) {
				this.queryActive_ = false;


				// If not aborting from a failure, on the whole article, in the future, 
				// make sure to still push an answer for the section, but include an error status so it can be filtered out

				if (this.debug_) console.warn("Error with section", typeof(error), error)
		...
		let output = this.PostProcess_(resultObj, outlineIsUserInput)
		this.queryActive_ = false;
